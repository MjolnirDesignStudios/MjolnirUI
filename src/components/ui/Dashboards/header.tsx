export function MjolnirHeader() {
  return (
    <header className="w-full flex items-center justify-between py-6 px-8 border-b border-zinc-800/50 bg-black/30 backdrop-blur-lg">
      <div className="font-black text-xl bg-linear-to-r from-cyan-300 to-emerald-400 bg-clip-text text-transparent">
        Dashboard
      </div>
      <div className="flex items-center gap-4">
        <button className="px-4 py-2 bg-linear-to-r from-cyan-500 to-emerald-500 text-black font-bold rounded-xl hover:brightness-110 transition">
          Upgrade
        </button>
        <button className="px-4 py-2 bg-zinc-900 text-white font-bold rounded-xl border border-zinc-700 hover:bg-zinc-800 transition">
          Sign Out
        </button>
      </div>
    </header>
  );
}
