...
export default function Navbar() {
  ...
  return (
    <motion.div
      ...
      className="flex fixed top-0 left-0 right-0 w-full z-50 py-6 bg-black backdrop-blur-xl border-b border-white/10"
    >
      <div className="max-w-7xl mx-auto w-full flex items-center px-6">
        {/* LOGO */}
          ...
          {navItems.map((item: { name: string; link: string }) => {
            ...
            return (
              <Link
                key={item.name}
                href={href}
                className="text-lg font-bold text-white hover:text-gold transition relative group decoration-none font-body"
              >
                {item.name}
                <motion.span
        ...
        <div className="ml-auto flex items-center gap-4">
          <button
            className="p-2.5 border border-white/20 rounded-lg bg-black/70 backdrop-blur-sm hover:bg-white/10 transition-all duration-300 group"
            title="Search"
            aria-label="Search"
          >
            <Search className="w-5 h-5 text-white group-hover:text-gold transition-colors" />
          </button>
          ...
          <button
            ...
            aria-label="Profile"
          >
            <User className="w-5 h-5 text-white group-hover:text-gold transition-colors" />
          </button>
