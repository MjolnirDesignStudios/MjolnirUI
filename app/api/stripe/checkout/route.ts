...
export async function POST(req: Request) {
  try {
    ...
    const { data: userData } = await supabaseAuthAdmin
      .from('users')
    ...
    const errorMessage = error?.message || error?.raw?.message || 'Checkout failed';
    console.error('Stripe checkout error:', errorMessage, error?.type, error?.statusCode);
    return NextResponse.json(
      { error: errorMessage },
