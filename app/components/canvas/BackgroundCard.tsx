// app/components/canvas/BackgroundCard.tsx
// Single card in the Backgrounds / Shader Backgrounds gallery.
// Shows a themed gradient placeholder (not a live render — perf reasons).
// Click → opens the BackgroundPreviewModal which does the live render.
"use client";

import React, { useState } from "react";
import { motion } from "framer-motion";
import { Copy, Check, Flame, LockKeyhole, Sparkles } from "lucide-react";
...
export function BackgroundCard({
  ...
  const handleCopyCmd = (e: React.MouseEvent) => {
    ...
    navigator.clipboard.writeText(`npx mjolnirui add ${entry.id}`);
    setCopied(true);
  ...
  return (
    ...
    >
      {/* ── Preview area (themed gradient placeholder) ───── */}
      <div
      ...
      <div className="p-4 flex-1 flex flex-col gap-2">
        ...
        >
          <span className="truncate">npx mjolnirui add {entry.id}</span>
          {isLocked ? (
